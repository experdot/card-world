...
import type { GameSetupConfig } from '@/shared/types';
import { WORLD_PRESETS, buildPlayerPreset, ABILITY_SET_INFO, type WorldPreset } from '@/data/presets';
...
export const SetupScreen: React.FC<SetupScreenProps> = ({
  ...
  const [selectedAbilitySets, setSelectedAbilitySets] = useState<string[]>(['basic']);
  ...
  const handleStart = () => {
    ...
    const presetElements = buildPlayerPreset(presetWithCustomSystems, selectedAbilitySets);
  ...
  const renderPresetSelection = () => (
    <div className="space-y-6">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-slate-500 text-sm">每个世界都有独特的背景、能力和挑战</p>
      </div>
  ...
  const renderCustomization = () => (
    <div className="space-y-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <h2 className="text-xl font-bold text-slate-200">定制: {selectedPreset?.name}</h2>
        <div className="w-16" />
      </div>

      {/* 世界设定 */}
      <div>
        <label className="block text-sm font-semibold text-slate-300 mb-2">世界背景</label>
        <textarea
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-semibold text-slate-300 mb-2">主角设定</label>
          <textarea
        ...
        <div>
          <label className="block text-sm font-semibold text-slate-300 mb-2">叙事风格</label>
          <textarea
            className="w-full h-20 bg-slate-950 border border-slate-700 rounded-lg p-3 text-sm focus:border-blue-500 outline-none resize-none"
      ...
      {/* 能力集合 */}
      <div>
        <label className="block text-sm font-semibold text-slate-300 mb-3">能力集合</label>
          ...
          {Object.entries(ABILITY_SET_INFO).map(([setId, info]) => (
            ...
            >
              <div className="flex items-center gap-2 mb-1">
                <span
                  className={
                    selectedAbilitySets.includes(setId) ? 'text-purple-400' : 'text-slate-500'
                  }
                >
                ...
                <span
                  className={`text-xs font-medium ${
                    selectedAbilitySets.includes(setId) ? 'text-purple-300' : 'text-slate-400'
                  }`}
                >
                  {info.name}
                </span>
              </div>
              <p className="text-[10px] text-slate-500 leading-tight">{info.description}</p>
            </button>
          ))}
        </div>
      </div>
      ...
      {/* 系统模块 */}
      <div>
        <label className="block text-sm font-semibold text-slate-300 mb-3">游戏系统</label>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
          <label
            className={`
            flex items-center gap-2 p-3 rounded-lg border cursor-pointer transition-all
            ${useInventory ? 'border-blue-500 bg-blue-900/20' : 'border-slate-700 bg-slate-800/50'}
          `}
          >
            <input
              type="checkbox"
              ...
              className="hidden"
            />
            <Backpack size={16} className={useInventory ? 'text-blue-400' : 'text-slate-500'} />
            <span className={`text-xs ${useInventory ? 'text-blue-300' : 'text-slate-400'}`}>
              背包
            </span>
          </label>
          ...
          <label
            className={`
            flex items-center gap-2 p-3 rounded-lg border cursor-pointer transition-all
            ${useEquipment ? 'border-blue-500 bg-blue-900/20' : 'border-slate-700 bg-slate-800/50'}
          `}
          >
            <input
              type="checkbox"
              ...
              className="hidden"
            />
            <Shield size={16} className={useEquipment ? 'text-blue-400' : 'text-slate-500'} />
            <span className={`text-xs ${useEquipment ? 'text-blue-300' : 'text-slate-400'}`}>
              装备栏
            </span>
          </label>
          ...
          <label
            className={`
            flex items-center gap-2 p-3 rounded-lg border cursor-pointer transition-all
            ${useStatus ? 'border-blue-500 bg-blue-900/20' : 'border-slate-700 bg-slate-800/50'}
          `}
          >
            <input
              type="checkbox"
              ...
              className="hidden"
            />
            <Heart size={16} className={useStatus ? 'text-blue-400' : 'text-slate-500'} />
            <span className={`text-xs ${useStatus ? 'text-blue-300' : 'text-slate-400'}`}>
              状态
            </span>
          </label>
          ...
          <label
            className={`
            flex items-center gap-2 p-3 rounded-lg border cursor-pointer transition-all
            ${useQuests ? 'border-blue-500 bg-blue-900/20' : 'border-slate-700 bg-slate-800/50'}
          `}
          >
            <input
              type="checkbox"
              ...
              className="hidden"
            />
            <Scroll size={16} className={useQuests ? 'text-blue-400' : 'text-slate-500'} />
            <span className={`text-xs ${useQuests ? 'text-blue-300' : 'text-slate-400'}`}>
              任务
            </span>
          </label>
        </div>
      ...
      <div className="p-4 bg-slate-800/50 rounded-xl border border-slate-700">
        <label className="flex items-center justify-between cursor-pointer">
          <div className="flex-1">
            <span className="text-sm font-semibold text-slate-300">跳过AI场景增强</span>
      ...
      {!hasLLMConfig && (
        <div className="mt-4 text-center">
          <p className="text-sm text-amber-500 mb-2">请先配置大模型才能开始游戏</p>
          <button
